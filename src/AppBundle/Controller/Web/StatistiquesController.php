...
//Disciplines des labos
        ...
        $query = $em->createQuery('SELECT d as item, COUNT(f.nom) as nb, f.nom as labo, f.id as id FROM AppBundle:Discipline d JOIN d.labo f GROUP BY d ORDER BY nb DESC')->setMaxResults(20);
        $allDisciplinesLabos = $query->getResult();

        //récupérer toutes les labos, leurs disciplines et le nombre de disciplines liées SISE
        $query = $em->createQuery('SELECT d as item, COUNT(f.nom) as nb, f.nom as labo, f.id as id FROM AppBundle:Discipline d JOIN d.labo f WHERE d.type=:type GROUP BY d ORDER BY nb DESC')->setMaxResults(20);
        $query->setParameter('type', 'SISE');
